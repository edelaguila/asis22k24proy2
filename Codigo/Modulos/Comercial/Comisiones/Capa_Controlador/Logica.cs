...
namespace Capa_Controlador
{
    public class Logica
    {
        ...
        {
            public string sIdFactura { get; set; }         // ID de la factura asociada al detalle de comisión
        ...
        public void funGuardarComision(int iIdVendedor, decimal deTotalVenta, decimal deTotalComision, List<DetalleComision> detalles)
        {
            try
            {
                ...
                {
                    sn.funInsertarComisionDetalle(idComisionEnc, detalle.sIdFactura, detalle.dePorcentaje, detalle.deMontoVenta, detalle.deMontoComision);
                }
            }
            catch (Exception ex)
            {
        ...
        public bool funGuardarComision(int iIdVendedor, DateTime dFecha, decimal deTotalVentas, decimal deTotalComision, decimal dePorcentajeComision, List<string> sIdVentas)
        {
            try
            {
                ...
                if (iIdComisionEnc > 0)
                {
                    ...
                    foreach (string sIdFactura in sIdVentas)
                    {
                        // Calcular la parte proporcional de la comisión para cada venta
                        decimal deMontoComision = (deTotalVentas / sIdVentas.Count) * dePorcentajeComision;
                        decimal deMontoVentaIndividual = deTotalVentas / sIdVentas.Count;

                        sn.funInsertarDetalleComision(iIdComisionEnc, sIdFactura, dePorcentajeComision, deMontoVentaIndividual, deMontoComision);
                    }
                    return true;
                }
                return false;
