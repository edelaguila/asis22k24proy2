...
namespace Capa_Modelo_Anticipo
{
    public class Sentencias
    {
        ...
        public void CalcularAnticipo(string mes)
        {
            using (OdbcConnection connection = cn.conexion())
            {
                ...
                while (readerAnticipos.Read())
                {
                    ...
                    string queryInsert = "INSERT INTO tbl_dedu_perp_emp (Fk_clave_empleado, Fk_dedu_perp, dedu_perp_emp_cantidad, estado) VALUES (?, ?, ?, 1) ON DUPLICATE KEY UPDATE dedu_perp_emp_cantidad = VALUES(dedu_perp_emp_cantidad), estado = 1;";
